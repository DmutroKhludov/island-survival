...
// размер вьюпорта = всё окно (полноэкранный режим)
...
// детерминированный ГПСЧ (mulberry32): одинаковый мир у всех игроков по общему SEED
let _rngState = 1;
function seedRng(s) { _rngState = (Math.floor(s * 100000) >>> 0) || 1; }
function srand() {
...
==================================================================== */
let SEED = Math.random() * 1000;   // может переопределяться сервером (общий остров в мультиплеере)
function hash(x, z) {
...
// подгоняем рендер и камеру под размер окна
function resize() {
...
function spawnPalms() {
  if (!palmProto) return;
  seedRng(SEED + 7);                        // отдельный детерминированный поток для пальм
  const N = 16;
  let pid = 100000;                         // диапазон id пальм (не пересекается со scatter)
  for (let i = 0; i < N; i++) {
    const ang = (i / N) * Math.PI * 2 + srange(-0.16, 0.16);
    const rfac = srange(0.2, 0.65);
    const rotY = srange(0, 6.28), swayV = srange(0, 6.28);
    pid++;
    const { rGrass, rWater } = beachSpan(ang);
    if (rGrass == null || rWater == null || rWater - rGrass < 1.5) continue;
    const r = lerp(rGrass, rWater, rfac);              // на песке, чуть в глубине от воды
...
function addEntity(type, x, z, opts = {}) {
  ...
  scene.add(obj);
  const id = opts.id !== undefined ? opts.id : _localEntId++;
  const ent = Object.assign({ type, x, z, obj, id }, e);
  obj.userData.entity = ent;        // чтобы луч прицела находил сущность по мешу
  game.entities.push(ent);
  return ent;
}
let _localEntId = 1000000;          // id для локальных объектов (костры) — вне диапазона мира
...
function scatterWorld() {
  // деревья / камни / кусты на суше — детерминированно от SEED (общий мир)
  seedRng(SEED + 1);
  let placed = 0, tries = 0, gid = 0;
  const want = { tree: 130, rock: 55, bush: 70, grass: 90 };
  const counts = { tree: 0, rock: 0, bush: 0, grass: 0 };
  while (tries < 9000 && placed < 330) {
    tries++;
    const x = srange(-ISLAND_R, ISLAND_R), z = srange(-ISLAND_R, ISLAND_R);
    ...
    addEntity(type, x, z, { id: gid++, rot: srange(0, Math.PI * 2) });
    counts[type]++; placed++;
  }
  applyHarvested();           // убрать объекты, уже сломанные другими игроками
}
...
// захват указателя → камера крутится бесконечно (курсор не упирается в край окна)
...
function setupControls() {
  ...
  document.addEventListener('pointerlockchange', () => {
    ...
    updateCursor();
    // подсказка, что курсор отпущен и надо кликнуть для возврата управления
    if (game.running && !camCtl.locked && !menuOpen()) showPrompt('🖱 Кликни — вернуть управление камерой');
  });
...
const JUMP_SPEED = 6.5;           // начальная скорость прыжка (≈1.2 м высоты)
const GRAVITY = 18;               // м/с²
const _UP = new THREE.Vector3(0, 1, 0);

// вертикаль: прыжок (пробел) и гравитация; floor — высота поверхности под игроком
function verticalUpdate(p, dt, floor) {
  const grounded = p.y <= floor + 0.05 && p.vy <= 0.01;
  if (grounded) {
    p.y = floor;
    p.vy = keys[' '] ? JUMP_SPEED : 0;   // прыжок с земли/палубы (удержание = подскоки)
  } else {
    p.vy -= GRAVITY * dt;
    p.y += p.vy * dt;
...
function movePlayer(dt) {
  ...
  if (p.mode === 'deck') {
    ...
    verticalUpdate(p, dt, game.deck.deckY);
    tryDescend(p);
  } else {
    if (!blocked(nx, p.z)) p.x = nx;
    if (!blocked(p.x, nz)) p.z = nz;
    verticalUpdate(p, dt, terrainHeight(p.x, p.z));
    const L = nearestLadder(p.x, p.z, LADDER_GRAB);
    if (L && (keys['w'] || keys['arrowup'])) {     // у основания и жмём вверх → лезем
      p.mode = 'climb'; p.ladder = L; p.x = L.bx; p.z = L.bz; p.y = Math.max(p.y, L.baseY); p.vy = 0;
    }
  }
...
// поднимаемся по иерархии меша до записи о пикапе (мешке)
function rootPickup(o) {
  while (o) { if (o.userData && o.userData.pickup) return o.userData.pickup; o = o.parent; }
  return null;
}
...
function updateNear() {
  ...
  // наведение на мешок — показать содержимое (подбор всё равно автоматический при подходе)
  const bagRoots = [];
  for (const d of pickups) if (d.kind === 'bag' && d.obj) bagRoots.push(d.obj);
  if (bagRoots.length) {
    for (const h of _raycaster.intersectObjects(bagRoots, true)) {
      const pk = rootPickup(h.object);
      if (pk) {
        game.near = null;
        showPrompt(`🛍 ${ITEMS[pk.item].icon} ${ITEMS[pk.item].name} ×${pk.count} — подойди забрать`);
        return;
      }
    }
  }
...
function doAction() {
  ...
  if (e.type === 'tree') {
    ...
    if (e.hp <= 0) {
      ...
      harvestRemove(e); log('🪵 Срубил дерево');
    }
  } else if (e.type === 'palm') {
    e.hp -= hasAxe ? 2 : 1;
    p.energy -= 3;
    ...
    if (e.hp <= 0) { give('stone', (rand(2,4)|0)+1); harvestRemove(e); log('🪨 Добыл камень'); }
  } else if (e.type === 'bush') {
    if (e.berries > 0) {
      give('berry', e.berries); log(`🫐 Собрал ягоды (${e.berries})`);
      e.berries = 0; setBushBerries(e.obj, 0); e.regrow = 70; p.energy -= 1;
      netHarvest(e.id, 'berry');
    } else log('Ягоды ещё не созрели.');
  } else if (e.type === 'grass') {
    give('fiber', (rand(1,3)|0)); harvestRemove(e); p.energy -= 1; log('🌾 Собрал волокно');
  }
}
...
function interactCampfire(fire) {
  ...
  if ((game.inventory.wood|0) > 0) {
    ...
    netFuel(fire.id, fire.fuel);            // костёр продлится и у друга
...
// мешок с выброшенными предметами (узнаваемый холщовый куль)
function makeBag() {
  const g = new THREE.Group();
  const body = new THREE.Mesh(new THREE.SphereGeometry(0.26, 10, 10), toon(0xcaa46c));
  body.scale.set(1, 1.08, 1); body.position.y = 0.24; body.castShadow = true;
  g.add(body);
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.15, 0.13, 8), toon(0xb18a52));
  neck.position.y = 0.48; g.add(neck);
  const tie = new THREE.Mesh(new THREE.TorusGeometry(0.1, 0.028, 6, 10), toon(0x6e4a28));
  tie.rotation.x = Math.PI / 2; tie.position.y = 0.46; g.add(tie);
  return g;
}

const pickups = [];
// item, x, z, fromY + опции: { count, kind:'coconut'|'bag', pid, synced }
function dropPickup(item, x, z, fromY, opts = {}) {
  const kind = opts.kind || 'coconut';
  const count = opts.count || 1;
  const obj = kind === 'bag' ? makeBag() : makeCoconut();
  obj.position.set(x, fromY, z);
  scene.add(obj);
  const d = { obj, item, count, kind, x, z, y: fromY, landed: false, bob: rand(0, 6.28),
              pid: opts.pid, synced: !!opts.synced, age: 0 };
  if (kind === 'bag') {                 // мешок падает строго на месте (одинаково у всех)
    d.vx = 0; d.vz = 0; d.vy = 2.4;
  } else {
    const a = rand(0, 6.28), spd = rand(0.7, 1.7);
    d.vx = Math.cos(a) * spd; d.vz = Math.sin(a) * spd; d.vy = rand(1.5, 3);
  }
  obj.userData.pickup = d;              // чтобы прицел «читал» содержимое мешка
  pickups.push(d);
  return d;
}
...
function updatePickups(dt, t) {
  ...
  for (let i = pickups.length - 1; i >= 0; i--) {
    const d = pickups[i];
    d.age += dt;                       // защита от мгновенного подбора своего же мешка
    if (!d.landed) {
      ...
      if (game.running && p && d.age > 0.6 && dist2(p.x, p.z, d.x, d.z) < 1.6 * 1.6 && canTake(d.item)) {
        give(d.item, d.count);
        const suff = d.count > 1 ? ` ×${d.count}` : '';
        log(`${ITEMS[d.item].icon} Подобрал ${ITEMS[d.item].name}${suff}`);
        if (d.synced && d.pid != null) netPickup(d.pid);   // сообщить — мешок забрали
        removePickup(i);
      }
    }
  }
...
// убрать мешок по сетевому id (его подобрал другой игрок)
function removePickupByPid(pid) {
  const i = pickups.findIndex(d => d.pid === pid);
  if (i >= 0) removePickup(i);
}

// выбросить весь стак из слота инвентаря в мир (мешком перед игроком)
function dropItemToWorld(src) {
  const arr = getSlotArray(src.arr);
  const item = arr[src.idx];
  if (!item) return;
  if (item === 'backpack') { log('🎒 Рюкзак нельзя выбросить'); return; }
  const count = game.inventory[item] | 0;
  if (count <= 0) return;
  const p = game.player;
  const x = p.x + Math.sin(p.dir) * 2.2, z = p.z + Math.cos(p.dir) * 2.2;  // дальше радиуса подбора
  // убрать из инвентаря
  game.inventory[item] = 0;
  arr[src.idx] = null;
  const pid = nextDropId();
  dropPickup(item, x, z, 1.2, { count, kind: 'bag', pid, synced: true });
  netDrop(pid, item, count, x, z);
  log(`Выбросил ${ITEMS[item].icon} ${ITEMS[item].name} ×${count}`);
  renderInventory();
}
...
function placeItem(item) {
  ...
  game.inventory[item]--;
  if (item === 'campfire') {
    const id = nextPlaceId(), rot = rand(0, Math.PI * 2);
    addEntity('campfire', x, z, { id, rot });
    netPlace('campfire', x, z, id, rot);     // показать костёр у друга
    log('🔥 Развёл костёр');
  }
  renderInventory();
}
...
let dragSource = null;
let dragHandled = false;     // предмет лёг в слот (а не выброшен наружу)

// курсор за пределами всего инвентарного UI (хотбар + окно «Инвентарь»)?
function pointerOutsideInventory(x, y) {
  const inRect = (el) => {
    if (!el) return false;
  ...
  const inBag = bag && !bag.classList.contains('hidden') && inRect(bag);
  return !inBag && !inRect(document.getElementById('inventory'));
}
...
function setupSlotDrag(el, arrName, idx) {
  ...
  el.addEventListener('dragend', e => {
    ...
    // не попал ни в один слот и отпущен вне инвентаря → выбросить в мир
    if (!dragHandled && dragSource && pointerOutsideInventory(e.clientX, e.clientY)) {
      dropItemToWorld(dragSource);
    }
    dragSource = null;
  });
...
function toggleCraft() {
  ...
  else if (game.running) lockPointer();        // закрыли меню — вернуть захват камеры
  updateCursor();
}
function toggleBag() {
  ...
  else if (game.running) lockPointer();        // закрыли инвентарь — вернуть захват камеры
  updateCursor();
}
...
function loop(ts) {
  ...
  // мир строится асинхронно (после seed от сервера) — ждём игрока
  if (!game.player) { renderer.render(scene, camera); requestAnimationFrame(loop); return; }
  if (game.running) {
    movePlayer(dt);
    updateNear();
    updateLadderPrompt();
...
==================================================================== */
function startGame() {
  buildWorld();          // гарантируем рельеф/воду/корабль (если seed ещё не пришёл)
...
// убрать все объекты, уже сломанные другими (вызывается после генерации мира)
function applyHarvested() {
  for (const id of net.harvested) removeById(id);
}
// сообщить серверу о добыче ('remove' — навсегда, 'berry' — собрали ягоды)
function netHarvest(id, action) {
  if (action !== 'berry') net.harvested.add(id);
  if (!net.connected || !net.ws || net.ws.readyState !== 1) return;
  net.ws.send(JSON.stringify({ type: 'harvest', id, action: action || 'remove' }));
}
// локально сломать объект + разослать другим
function harvestRemove(e) { netHarvest(e.id, 'remove'); removeEntity(e); }

// уникальный id выброшенного мешка (свой диапазон у каждого игрока)
let _dropCounter = 0;
function nextDropId() { return (net.id || 999) * 1000000 + (++_dropCounter); }
function netDrop(pid, item, count, x, z) {
  if (!net.connected || !net.ws || net.ws.readyState !== 1) return;
  net.ws.send(JSON.stringify({ type: 'drop', pid, item, count, x, z }));
}
function netPickup(pid) {
  if (!net.connected || !net.ws || net.ws.readyState !== 1) return;
  net.ws.send(JSON.stringify({ type: 'pickup', pid }));
}
// показать чужой мешок (или мешки из init у опоздавшего)
function spawnRemoteDrop(m) {
  if (pickups.some(d => d.pid === m.pid)) return;
  dropPickup(m.item, m.x, m.z, 1.2, { count: m.count, kind: 'bag', pid: m.pid, synced: true });
}

// id поставленного объекта (костра) — уникален у каждого игрока, не пересекается с миром
let _placeCounter = 0;
function nextPlaceId() { return 5000000 + (net.id || 0) * 100000 + (++_placeCounter); }
function netPlace(objType, x, z, id, rot) {
  net.placed.set(id, { objType, x, z, id, rot });
  if (!net.connected || !net.ws || net.ws.readyState !== 1) return;
  net.ws.send(JSON.stringify({ type: 'place', objType, x, z, id, rot }));
}
// создать поставленный объект из сетевой записи (с учётом топлива костра)
function addPlacedEntity(m) {
  if (game.entities.some(e => e.id === m.id)) return null;   // уже стоит
  if (m.objType !== 'campfire') return null;
  const e = addEntity('campfire', m.x, m.z, { id: m.id, rot: m.rot });
  if (m.fuel != null) e.fuel = m.fuel;
  if (m.lit != null) e.lit = m.lit;
  return e;
}
// поставить чужой объект (или восстановить после генерации мира)
function spawnRemotePlace(m) {
  net.placed.set(m.id, m);
  addPlacedEntity(m);
}
// заново разместить все известные костры (после рестарта/перегенерации мира)
function applyPlaced() {
  for (const m of net.placed.values()) addPlacedEntity(m);
}
// подкинули дров — разослать новое значение топлива
function netFuel(id, fuel) {
  const m = net.placed.get(id);
  if (m) { m.fuel = fuel; m.lit = true; }
  if (!net.connected || !net.ws || net.ws.readyState !== 1) return;
...
function updateRemotes(dt) {
  ...
  for (const r of net.others.values()) {
    ...
    r.z = lerp(r.z, r.tz, k);
    r.y = lerp(r.y, r.ty, k);
    r.dir = lerpAngle(r.dir, r.tdir, k);
    r.obj.position.set(r.x, r.y, r.z);
    r.obj.rotation.y = r.dir;
    if (r.moving) r.step += dt * 8;
    const u = r.obj.userData;
    if (u && u.legL) {
      const sw = r.moving ? Math.sin(r.step) * 0.6 : 0;
      u.legL.rotation.x = sw; u.legR.rotation.x = -sw;
      u.armL.rotation.x = -sw; u.armR.rotation.x = sw;
    }
...
function netSendMove() {
  ...
  if (now - lastSent < 66) return;          // ~15 пакетов/сек
  lastSent = now;
  const p = game.player;
...
function connectMultiplayer() {
  ...
  catch (_) { buildWorld(); return; }
  net.ws = ws;
  let settled = false;
  // офлайн-мир строим ТОЛЬКО если сервера реально нет (иначе ждём seed, чтобы остров совпал)
  const goOffline = () => { if (!settled) { settled = true; buildWorld(); } };
  // защита от зависшего сервера: если за 8 с нет init — играем офлайн
  const fallback = setTimeout(goOffline, 8000);
  ...
  ws.onmessage = (ev) => {
    ...
    if (msg.type === 'init') {
      ...
      net.connected = true;
      net.id = msg.id;
      SEED = msg.seed;                       // общий остров для всех
      net.harvested = new Set(msg.harvested || []);   // что уже сломали до нас
      buildWorld();
      applyHarvested();
  ...
  // нет сервера (или связь оборвалась до init) — строим одиночный мир
  ws.onclose = () => { net.connected = false; clearTimeout(fallback); goOffline(); };
  ws.onerror = () => {};
}
...
==================================================================== */
initThree();
connectMultiplayer();   // строит мир после seed от сервера (или офлайн, если сервера нет)
...
function startWithFullscreen() {
  ...
  setTimeout(lockPointer, 250);        // повтор после перехода в полный экран
}
